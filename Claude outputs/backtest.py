...
def grade(rows, fixes, params):
    ...
    for r in rows:
        ...
        out.append({**r, "side": side, "conf": conf, "result": res, "abs_err": abs_err})
    return out
...
def main():
    ...
    print(f"{'='*78}")
    print(f"  {'Stage':24} {'All games':>16} {'Top 5':>16} {'Above median':>16} {'MAE':>5}")
    ...
    for i, (label, fixes) in enumerate(STAGES):
        ...
        if i == 0:
            line_mae = [abs(-r["spread_home"] - r["margin"]) for r in rows]
            if mism:
        ...
        mae = f"{s['margin_mae']:.1f}" if s["margin_mae"] is not None else "  -"
        print(f"  {label:24} {f(s['all']):>16} {f(s['top5']):>16} {f(s['above_median']):>16} {mae:>5}")
        report["stages"].append({"stage": label, "fixes": list(fixes), **s})

    print(f"\n  Vegas line MAE (how far the spread misses the final margin): "
          f"{sum(line_mae)/len(line_mae):.1f}")
